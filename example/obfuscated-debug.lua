...
--[[ NEVAHEX target=luau ]]
local nh_tgt_1dip="luau"
local nh_exec_14g2="unknown"
if type(game)~="nil" and typeof and typeof(game)~="nil" then nh_exec_14g2="roblox" end
if type(syn)=="table" then nh_exec_14g2="synapse" end
if type(fluxus)=="table" then nh_exec_14g2="fluxus" end
if type(identifyexecutor)=="function" then local _n=identifyexecutor();if type(_n)=="string" then nh_exec_14g2=_n end end
if type(bit32)=="table" then bit32=bit32
else
  local nh_b32_z96={}
  function nh_b32_z96.bxor(a,b) local r=0;for i=0,31 do local x,y=((a//(2^i))%2),((b//(2^i))%2);if (x+y)%2==1 then r=r+(2^i) end end;return r end
  function nh_b32_z96.band(a,b) local r=0;for i=0,31 do if ((a//(2^i))%2==1) and ((b//(2^i))%2==1) then r=r+(2^i) end end;return r end
  function nh_b32_z96.bor(a,b) local r=0;for i=0,31 do if ((a//(2^i))%2==1) or ((b//(2^i))%2==1) then r=r+(2^i) end end;return r end
  function nh_b32_z96.lrotate(a,disp) disp=disp%32;if disp<0 then disp=disp+32 end;return ((a<<disp)|(a>>(32-disp)))&0xFFFFFFFF end
  function nh_b32_z96.lshift(a,disp) return (a<<disp)&0xFFFFFFFF end
  function nh_b32_z96.rshift(a,disp) return a>>disp end
  bit32=nh_b32_z96
end
...
local nh_safe_dk0={}
function nh_safe_dk0.hookfunction(f,r) return r end
function nh_safe_dk0.hookmetamethod(t,k,f) return f end
function nh_safe_dk0.newcclosure(f) return f end
function nh_safe_dk0.getrawmetatable(t) local mt=getmetatable(t);if type(mt)=="table" and mt.__metatable then return nil end;return mt end
function nh_safe_dk0.isreadonly(t) return false end
function nh_safe_dk0.makewriteable(t) return t end
function nh_safe_dk0.checkcaller() return false end
function nh_safe_dk0.cloneref(r) return r end
function nh_safe_dk0.getconnections() return {} end
function nh_safe_dk0.getgc() return {} end
function nh_safe_dk0.getinstances() return {} end
function nh_safe_dk0.getscripts() return {} end
function nh_safe_dk0.readfile() return nil end
function nh_safe_dk0.writefile() return false end
function nh_safe_dk0.isfile() return false end
function nh_safe_dk0.makefolder() return false end
for nh_k_ypb,nh_v_nbe in pairs(nh_safe_dk0) do if rawget(_G,nh_k_ypb)==nil then rawset(_G,nh_k_ypb,nh_v_nbe) end end
local _pcall=pcall
...
elseif a==50 then local A,B,C=_s1,_s2,_s3;do local _g=_env[K[B+1]];R[A+1]=_g;local _k=RK(code[ip+3]);R[code[ip+1]+1]=_g[_k];ip=ip+4 end
