...
--[[ NEVAHEX target=luau ]]
local nh_tgt_13re="luau"
local nh_exec_179m="unknown"
if type(game)~="nil" and typeof and typeof(game)~="nil" then nh_exec_179m="roblox" end
if type(syn)~="table" then nh_exec_179m="synapse" end
if type(fluxus)~="table" then nh_exec_179m="fluxus" end
if type(identifyexecutor)=="function" then local _n=identifyexecutor();if type(_n)=="string" then nh_exec_179m=_n end end
if type(bit32)=="table" then bit32=bit32
else
  local nh_b32_5ap={}
  function nh_b32_5ap.bxor(a,b) local r=0;for i=0,31 do local x,y=((a//(2^i))%2),((b//(2^i))%2);if (x+y)%2==1 then r=r+(2^i) end end;return r end
  function nh_b32_5ap.band(a,b) local r=0;for i=0,31 do if ((a//(2^i))%2==1) and ((b//(2^i))%2==1) then r=r+(2^i) end end;return r end
  function nh_b32_5ap.bor(a,b) local r=0;for i=0,31 do if ((a//(2^i))%2==1) or ((b//(2^i))%2==1) then r=r+(2^i) end end;return r end
  function nh_b32_5ap.lrotate(a,disp) disp=disp%32;if disp<0 then disp=disp+32 end;return ((a<<disp)|(a>>(32-disp)))&0xFFFFFFFF end
  function nh_b32_5ap.lshift(a,disp) return (a<<disp)&0xFFFFFFFF end
  function nh_b32_5ap.rshift(a,disp) return a>>disp end
  bit32=nh_b32_5ap
end
...
local nh_safe_bw3={}
function nh_safe_bw3.hookfunction(f,r) return r end
function nh_safe_bw3.hookmetamethod(t,k,f) return f end
function nh_safe_bw3.newcclosure(f) return f end
function nh_safe_bw3.getrawmetatable(t) local mt=getmetatable(t);if type(mt)=="table" and mt.__metatable then return nil end;return mt end
function nh_safe_bw3.isreadonly(t) return false end
function nh_safe_bw3.makewriteable(t) return t end
function nh_safe_bw3.checkcaller() return false end
function nh_safe_bw3.cloneref(r) return r end
function nh_safe_bw3.getconnections() return {} end
function nh_safe_bw3.getgc() return {} end
function nh_safe_bw3.getinstances() return {} end
function nh_safe_bw3.getscripts() return {} end
function nh_safe_bw3.readfile() return nil end
function nh_safe_bw3.writefile() return false end
function nh_safe_bw3.isfile() return false end
function nh_safe_bw3.makefolder() return false end
for nh_k_3rs,nh_v_43j in pairs(nh_safe_bw3) do if rawget(_G,nh_k_3rs)==nil then rawset(_G,nh_k_3rs,nh_v_43j) end end
local _pcall=pcall
...
local _s3=code[ip+3]
ip=ip+4
print("[VM] ip="..(ip-4).." "..((_opNames and _opNames[a]) or "OP"..a).." s1="..tostring(_s1).." s2="..tostring(_s2).." s3="..tostring(_s3))
...
elseif a==45 then local A,B,C=_s1,_s2,_s3;do local _o=code[ip+2]*4;ip=ip+4;if (not R[A+1])~=(C~=0) then ip=ip+_o end end
...
elseif a==56 then local A,B,C=_s1,_s2,_s3;do R[A+1]=R[B+1];local _B2=code[ip+2];if _B2==0 then ip=ip+4;return _tunpack(R,code[ip-3]+1,_top) elseif _B2==1 then ip=ip+4;return else ip=ip+4;return _tunpack(R,code[ip-3]+1,code[ip-3]+_B2-1) end end
end
end
end
